...
- **Strategy:** Replace individual edge case tests with hypothesis-based property tests
- **Outcome:** keep
- **Insight:** A single property test covers more cases than 5 individual tests
- **Context:** Goal=increase test coverage, Scope=tests/**/*.test.ts, Metric=pass count (higher)
- **Iteration:** 14
- **Run:** autoresearch-20260320T143022
- **Timestamp:** 2026-03-20T15:12:44Z
...
### Lesson: Adding tests one-by-one has diminishing returns

- **Strategy:** Add individual test cases for uncovered code paths
- **Outcome:** pivot
- **Insight:** After ~10 individual tests, each new test only improves metric by 1. Refactoring for testability yields larger per-iteration gains.
- **Context:** Goal=increase test coverage, Scope=tests/**/*.test.ts, Metric=pass count (higher)
- **Iteration:** 22
- **Run:** autoresearch-20260320T143022
- **Timestamp:** 2026-03-20T15:45:11Z
...
### Lesson: Bundle splitting reduces size more than code minification

- **Strategy:** Configure code splitting at route boundaries
- **Outcome:** keep
- **Insight:** Splitting reduced bundle by 40% whereas manual minification only achieved 5%. The build tool already minifies; splitting is the leverage point.
- **Context:** Goal=reduce bundle size, Scope=src/**/*.ts, Metric=dist size in KB (lower)
- **Iteration:** 7
- **Run:** autoresearch-20260319T091500
